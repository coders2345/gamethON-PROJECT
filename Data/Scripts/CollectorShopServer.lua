...
function InventorySellAll(player)
    ...
    if API_MEMBERSHIP.IsRegistered() then
        ...
        bonusAmount = membersCount * API_MEMBERSHIP.GetSellBonus(API_MEMBERSHIP.ITEM_SELL_BONUS)
    end

    for itemId, count in pairs(INVENTORY.GetAllItems(player)) do
        if not string.match(ITEM_LIBRARY.GetItem(itemId).area, "FISHING") then
    ...
    if EventAPI.IsEventKeyActive("2XMETAL") then
        total = total * 2
    end
    ...
    _G.AnalyticsFacade.TrackSoldItems(player, "SoldCollectables")
end

Events.ConnectForPlayer("CSell", InventorySellAll)
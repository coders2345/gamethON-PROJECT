...
function Tick()
    ...
    if (currentState == API.STATE.AVAILABLE and not ingoredInvite) or API.GetParticipatingPlayerCount() == 0 then
    ...
    elseif currentState == API.STATE.COUNTDOWN then
        ...
        MESSAGE.text = "Destroy Everyone!"
    else
        INVITATION.visibility = Visibility.FORCE_OFF
        RUNNING.visibility = Visibility.FORCE_OFF
        --if playerWasParticipating then
            if currentState == API.STATE.RUNNING then
...
function OnBalloonWinner(winnerPlayer)
    --if not playerWasParticipating then return end
    VICTORY.visibility = Visibility.FORCE_ON
    WINNER_NAME.text = "Unkown"
...
local balloonWinnerHandle = Events.Connect("AnnounceBalloonRoyaleWinner", OnBalloonWinner)

ROOT.destroyEvent:Connect(function()
    balloonWinnerHandle:Disconnect()
end)
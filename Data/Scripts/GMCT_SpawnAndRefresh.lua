...
function CMD.Execute_Server(sourcePlayer, targetPlayer, obj)
    ...
    if data.spawnedProps[targetPlayer.id] ~= nil then
        if Object.IsValid(data.spawnedProps[targetPlayer.id] ) then 
        ...
        data.spawnedProps[targetPlayer.id] =
            World.SpawnAsset(
            propId,
            {
                position = targetPlayer:GetWorldPosition() + targetPlayer:GetLookWorldRotation() * data.offset
            }
        )
        if data.isEquipment then
            data.spawnedProps[targetPlayer.id]:Equip(targetPlayer)
        end
        if data.hidePlayer then
        ...
        if obj:GetCustomProperty('AttachToPlayerSocket') ~= nil then
            data.spawnedProps[targetPlayer.id]:AttachToPlayer(
                targetPlayer,
                obj:GetCustomProperty('AttachToPlayerSocket')
            )
        end 
        targetPlayer:Spawn()

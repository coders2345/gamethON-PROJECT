...
local propFirenadoStatus = script:GetCustomProperty("FirenadoStatus")
...
function OnHitSomething(trigger, other)
  if other:IsA("Player") then
    ...
    if not player.isDead then
      ...
      local status = World.SpawnAsset(propFirenadoStatus)
      status.serverUserData.targetPlayer = player
      status:AttachToPlayer(player, "pelvis")

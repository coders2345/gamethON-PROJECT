...
local JEDI_WEAPON = script:GetCustomProperty("BalloonWeapon")
local JEDI_ROYALE_KIT = script:GetCustomProperty("JediRoyaleKit")
...
local function SetupPlayerForGame(player)
    local weapon = World.SpawnAsset(JEDI_WEAPON)
    ...
    -- If the player dies for any reason.
    if player.serverUserData.jediDeathHandle then return end
    local deathHandle = nil
    deathHandle = player.diedEvent:Connect(function(_,damage)
        OnBalloonShot(damage.sourcePlayer,player)
    end)
    player.serverUserData.jediDeathHandle = deathHandle
end

function OnJoinJediRoyale(player)
...
ROOT.destroyEvent:Connect(function(_)
    ...
    for _, player in pairs(allPlayers) do
        if Object.IsValid(player) then
            player:SetResource(API.GetResourceName(),0)
            -- Disconnect death handles.
            if player.serverUserData.jediDeathHandle then
                player.serverUserData.jediDeathHandle:Disconnect()
                player.serverUserData.jediDeathHandle = nil
            end
...
local jediKitRoyale = World.SpawnAsset(JEDI_ROYALE_KIT)
...
local askJoinHandle = Events.ConnectForPlayer("AskJoinBalloonRoyale", OnJoinJediRoyale)

-- Locally for when a request comes in from the server
local joinRoyaleEventHandle = Events.Connect("JoinBalloonRoyale", OnJoinJediRoyale)

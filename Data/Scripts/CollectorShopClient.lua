...
function OnSell()
    ...
    local res, msg = Events.BroadcastToServer("CSell")
...
function ConstructPanel()
    ...
    for id, itemCount in pairs(INVENTORY.GetAllItems(LOCAL_PLAYER)) do
        ...
        if not string.match(item.area, "FISHING") then
...
function ToggleUI (uniqueId)
    if uniqueId == "CollectorVendorUI" then
        ConstructPanel()
    end
end
...
function HandleInteracted(trigger, other)
    if other:IsA("Player") then
        ...
        Events.Broadcast("ToggleUI","CollectorVendorUI", true)
	end
end
